...
import {
    ...
    Tool, LLMConfig, LLMRequest,
    LLMResponse, Message, ToolMessage,
    LLMProvider, ChatCompletionTool,
    ToolCallResult
...
export class LLMV2Service {
    ...
    private async callOpenAIWithStructuredOutputWithTools(request: LLMRequest, messages: Message[]): Promise<LLMResponse> {
    ...
    async generateResponseWithStructuredOutputNoTools(request: LLMRequest): Promise<LLMResponse> {
        // Validate functionInput as LLMRequest V1
        try {
            request = await this.validationPipe.transform(request, { metatype: LLMRequest, type: 'body' });
    ...
    async generateResponseWithStructuredOutputWithTools(request: LLMRequest): Promise<LLMResponse> {
        // Validate functionInput as LLMRequest V1
        try {
            request = await this.validationPipe.transform(request, { metatype: LLMRequest, type: 'body' });
        } catch (validationError) {
